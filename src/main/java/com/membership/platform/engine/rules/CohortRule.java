...
public class CohortRule implements TierEligibilityRule {
    ...
    public int evaluate(EligibilityContext context) {
        String cohort = context.getUser().getCohort().name();
        if (properties.getCohort().getPlatinumCohorts().contains(cohort)) return 3;
        if (properties.getCohort().getGoldCohorts().contains(cohort)) return 2;
        return 1;
    }
}
